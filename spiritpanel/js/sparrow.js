...
(function (root, factory) {
  ...
  function buildInstance(parts, opts) {
    ...
    /* MXNestSpirit — coins Graphtec. Le moteur modifié (sparrow-mx) reçoit la
     * taille du carré à laisser vide à chaque coin de la bande et organise le
     * plan autour, pendant toute sa recherche. Un moteur d'origine ignore ce
     * champ ; la réparation du panneau (MXNest.guardCorners) reste derrière. */
    var inst = { name: "mxnestspirit", strip_height: stripHeight, items: items };
    /* La bande est élargie de padMm de chaque côté (voir plus haut) : le coin de
     * la planche est donc à padMm du coin de la bande. L'entaille le compte, et
     * le moteur ajoute de lui-même son écart habituel contre ce bord : les
     * pièces restent à la taille du carré PLUS l'écart de lame, comme ailleurs. */
    var notch = (o.cornerNotchMm || 0) > 0 ? o.cornerNotchMm + padMm : 0;
    if (notch > 0 && notch < stripHeight / 2) inst.corner_notch = notch;
    return {
      instance: inst,
      meta: {

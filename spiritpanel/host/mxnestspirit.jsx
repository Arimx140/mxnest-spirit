...
function mxnsPing() {
  try {
    var full = '';
    if (app.documents.length) { try { full = String(app.activeDocument.fullName || ''); } catch (eF) { full = ''; } }
    return 'OK\t' + app.name + '\t' + app.version + '\t' +
           (app.documents.length ? app.activeDocument.name : '-') + '\t' + full;
...
function mxnsScan(mode, namesCsv, selOnly, regroup, tight) {
  try {
    ...
    /* Un registre par document : analyser un deuxième onglet ne doit plus
       rendre inapplicable le plan calculé sur le premier. */
    if (!$.global.MXNS_REG) $.global.MXNS_REG = {};
    $.global.MXNS_REG[$.global.MXNS_DOC] = { items: $.global.MXNS_ITEMS, cuts: $.global.MXNS_CUTS,
                                            session: $.global.MXNS_SESSION };
...
function mxnsApply(payload, sheetWidth, lengthMm, fitArtboard, session) {
  try {
    if (!app.documents.length) return 'ERR\tAucun document ouvert';
    /* Chaque document garde ses références d'analyse : on reprend celles du
       document actif. Les contrôles ci-dessous restent les mêmes. */
    try {
      var keyNow = String(app.activeDocument.name) + '|' + String(app.activeDocument.fullName || '');
      var reg = $.global.MXNS_REG ? $.global.MXNS_REG[keyNow] : null;
      if (reg) {
        $.global.MXNS_ITEMS = reg.items; $.global.MXNS_CUTS = reg.cuts;
        $.global.MXNS_SESSION = reg.session; $.global.MXNS_DOC = keyNow;
      }
    } catch (eReg) {}
...
/* ================= remplissage par stickers (3 modèles) =================
   Le modèle peut être pris dans N'IMPORTE QUEL document ouvert (un fichier de
   stickers à côté du kit) : on garde sa référence, et les copies sont
   toujours posées dans le document ACTIF, sur un calque à part. */
var MXN_STICKLAYER = 'MXN_STICKERS';

function mxnsFillerPick(slot, tight) {
  try {
    if (!app.documents.length) return 'ERR\tAucun document ouvert';
    var doc = app.activeDocument;
    if (!doc.selection || !doc.selection.length) return 'ERR\tSélectionne le sticker dans Illustrator, puis reclique.';
    if (doc.selection.length > 1) return 'ERR\tPlusieurs objets sélectionnés : groupe-les d\'abord (Ctrl+G).';
    var it = doc.selection[0];
    var M = { mode: 'auto', names: [], key: '', tight: String(tight) === 'true' };
    var fp = footprint(it, M, 0.08);
    if (!fp.rings.length) return 'ERR\tContour du sticker introuvable.';
    if (!$.global.MXNS_FILL) $.global.MXNS_FILL = [null, null, null, null];
    var sl = Math.max(1, Math.min(3, parseInt(slot, 10) || 1));
    $.global.MXNS_FILL[sl] = { item: it, tight: M.tight };
    var gb = it.geometricBounds;
    var out = ['OK', 'F\t' + sl + '\t' + String(it.name || it.typename) + '\t' + fp.src + '\t' + (fp.union ? 1 : 0) +
               '\t' + ((gb[2] - gb[0]) / MM).toFixed(2) + '\t' + ((gb[1] - gb[3]) / MM).toFixed(2) + '\t' + String(doc.name)];
    for (var r = 0; r < fp.rings.length; r++) {
      var ring = fp.rings[r], buf = [];
      for (var q = 0; q < ring.length; q++) buf.push((Math.round(ring[q][0] * 100) / 100) + ',' + (Math.round(ring[q][1] * 100) / 100));
      out.push('R\t' + buf.join(' '));
    }
    return out.join('\n');
  } catch (e) { return 'ERR\t' + e + ' (ligne ' + e.line + ')'; }
...
function mxn_stickLayer(doc, fresh) {
  for (var L = doc.layers.length - 1; L >= 0; L--) {
    if (doc.layers[L].name !== MXN_STICKLAYER) continue;
    if (!fresh) { try { doc.layers[L].locked = false; } catch (e0) {} return doc.layers[L]; }
    try { doc.layers[L].locked = false; } catch (e1) {}
    doc.layers[L].remove();
  }
  var lay = doc.layers.add();
  lay.name = MXN_STICKLAYER;
  return lay;
}

/* payload : "slot,angle,scalePct,xmm,ymm;..."  x,y = coin haut-gauche visé du contour */
function mxnsApplyFillers(payload) {
  try {
    if (!app.documents.length) return 'ERR\tAucun document ouvert';
    if (!$.global.MXNS_FILL) return 'ERR\tAucun sticker chargé.';
    var doc = app.activeDocument;
    var lay = mxn_stickLayer(doc, true);          /* un seul lot à la fois */
    var ab = doc.artboards[doc.artboards.getActiveArtboardIndex()].artboardRect;
    var originX = ab[0], originY = ab[1];
    var rows = String(payload).split(';'), done = 0, dead = 0, firstErr = '';
    for (var i = 0; i < rows.length; i++) {
      if (!rows[i]) continue;
      var f = rows[i].split(',');
      var src = $.global.MXNS_FILL[parseInt(f[0], 10)];
      if (!src) continue;
      var it = src.item;
      try { var probe = it.typename; } catch (eDead) { dead++; continue; }
      var ang = parseFloat(f[1]), sc = parseFloat(f[2]), tx = parseFloat(f[3]), ty = parseFloat(f[4]);
      if (!(sc > 0)) sc = 100;
      var dup = null;
      try { dup = it.duplicate(lay, ElementPlacement.PLACEATEND); } catch (eDup) { if (!firstErr) firstErr = String(eDup); continue; }
      /* aucune étiquette d'analyse ne doit suivre la copie */
      try { dup.note = ''; } catch (eN) {}
      var inner = collect(dup, []);
      for (var k = 0; k < inner.length; k++) {
        try { var nt = String(inner[k].note || ''); if (nt.indexOf(TAG) === 0 || nt.indexOf(CUTTAG) === 0) inner[k].note = ''; } catch (eNi) {}
      }
      if (Math.abs(sc - 100) > 0.01) { try { dup.resize(sc, sc, true, true, true, true, sc, Transformation.CENTER); } catch (eRz) {} }
      if (ang) { try { dup.rotate(-ang, true, true, true, true, Transformation.CENTER); } catch (eRo) {} }
      /* recalage sur le même contour que celui du calcul, relu sur la copie */
      var Lb = null, Tb = null;
      try {
        var fp2 = footprint(dup, { mode: 'auto', names: [], key: '', tight: !!src.tight }, 0.08);
        for (var j = 0; fp2.cuts && j < fp2.cuts.length; j++) {
          try { var g2 = fp2.cuts[j].geometricBounds;
                if (Lb === null || g2[0] < Lb) Lb = g2[0]; if (Tb === null || g2[1] > Tb) Tb = g2[1]; } catch (eB) {}
        }
      } catch (eFp) {}
      if (Lb === null) { var gd = dup.geometricBounds; Lb = gd[0]; Tb = gd[1]; }
      dup.translate(originX + tx * MM - Lb, originY - ty * MM - Tb, true, true, true, true);
      try { dup.name = 'MXN_STICKER_' + f[0]; } catch (eNm) {}
      done++;
    }
    app.redraw();
    return 'OK\t' + done + '\t' + dead + '\t' + firstErr;
  } catch (e) { return 'ERR\t' + e + ' (ligne ' + e.line + ')'; }
}

function mxnsClearFillers() {
  try {
    if (!app.documents.length) return 'ERR\tAucun document ouvert';
    var doc = app.activeDocument, n = 0;
    for (var L = doc.layers.length - 1; L >= 0; L--) {
      if (doc.layers[L].name !== MXN_STICKLAYER) continue;
      try { doc.layers[L].locked = false; } catch (e1) {}
      n += doc.layers[L].pageItems.length;
